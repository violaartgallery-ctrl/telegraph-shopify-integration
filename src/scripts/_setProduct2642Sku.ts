/**
 * One-time SKU sync:
 * Shopify product 10481350869284 Photo Printing
 * Odoo product.template 2642 Photo Printing
 */
import { requestShopifyAdmin } from '../shopify/shopifyAdminGraphql.js';
import { OdooClient } from '../odoo/odooClient.js';
import { prisma } from '../lib/prisma.js';

const SHOPIFY_PRODUCT_GID = 'gid://shopify/Product/10481350869284';
const ODOO_TEMPLATE_ID = 2642;
const SKU = 'PRT-PHO';
...
if (conflicts.length > 0 && !conflicts.some((row) => String(row.display_name ?? '').includes('Photo Printing'))) {
...
if (shopify.product.title !== 'Photo Printing') {
  throw new Error(`Unexpected Shopify product title: ${shopify.product.title}`);
}
const shopifyVariants = shopify.product.variants.nodes;
if (shopifyVariants.length !== 1) throw new Error(`Expected 1 Shopify variant, found ${shopifyVariants.length}`);
...
`, {
  ...
  variants: [{ id: shopifyVariants[0].id, inventoryItem: { sku: SKU } }]
});
...
if (!template[0]) throw new Error(`Odoo template ${ODOO_TEMPLATE_ID} not found`);
if (template[0].name !== 'Photo Printing') {
  throw new Error(`Unexpected Odoo template name: ${template[0].name}`);
}
